...
def reconstructAll(img,h,step) :
    ...
    while len(allPatch["noisyPatch"])>0 :
        ...
        #allPatch["goodPatch"][newPatch[0]]=newPatch[1]
        #allPatch=getAllPatch(img,h,step)
    return img
...
def reconstructMissingPart(img,h,step,method="Naive"):
    ...
    while len(allPatch["noisyPatch"])>0 :
        ...
        _,newPatch = approximePatch(patchTarget,allPatch["goodPatch"],0.001)
...
def inpainting():
    ...
    #noisy_img=noise(original_img,0.3,[70,90,120,350])
    noisy_img=deleteRect(original_img,40,60,100,100) 
    h=13
    t=time.time()
